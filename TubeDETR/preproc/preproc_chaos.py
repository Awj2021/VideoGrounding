...
# load config
with open("config/chaos.json", "r") as f:
    cfg = json.load(f)
...
for dir in vidor_dirs:
    ...
    files = os.listdir(os.path.join(vidor_path, dir))
    for file in files:
        annot = json.load(open(os.path.join(vidor_path, dir, file), "r"))
        out = {
        ...
        for i_frame, traj in enumerate(annot["trajectories"]):
            for bbox in traj:
                ...
                assert bbox != [] # if the bbox is None, how to process.
